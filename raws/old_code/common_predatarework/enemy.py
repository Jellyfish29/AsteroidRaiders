...
class Enemy:
    ...
    def hit_detection(self, missile_hit):
        # Shots
        for shot, direction, dmg in tr.Turret.shot_lst:
            if self.hitbox.colliderect(shot):
                Gfx.shot_hit_effect(shot)
                self.health -= dmg
                self.healthbar_len -= (self.healthbar_max_len / (self.max_health / dmg))
                # Special Ammo
                if "piercing_shot" not in it.Items.active_flag_lst:
                    tr.Turret.shot_lst.remove((shot, direction, dmg))
        ...
        if self.hitbox.colliderect(tr.Turret.nuke):
            self.health -= 1.5
            self.healthbar_len -= (self.healthbar_max_len / (self.max_health / 1.5))

        # Missile
        for missile, _ in tr.Turret.missile_lst:
            if self.hitbox.colliderect(missile):
                self.health -= pl.Player.damage * 3
                self.healthbar_len -= (self.healthbar_max_len / (self.max_health / (pl.Player.damage * 3)))
                Gfx.create_effect("shot_hit", 7, (missile.topleft[0] - 10, missile.topleft[1] - 10))
                tr.Turret.missile_lst.remove((missile, _))
        # He Rounds
        for explo, dmg in tr.Turret.he_rounds_lst:
            if self.hitbox.colliderect(explo):
                # Gfx.explosion effect
                self.health -= dmg
                self.healthbar_len -= (self.healthbar_max_len / (self.max_health / dmg))
        ...
        if self.health <= 0:
            if self.__class__ == Enemy:
                lvl.Levels.wormhole_spawn_time -= Enemy.ttk_ticker
            else:
                lvl.Levels.wormhole_spawn_time -= Enemy.ttk_ticker + 20
            Enemy.ttk_ticker = 100

            # On Death Item Effects
            tr.Turret.overdrive()
    ...
    def gfx_animation(self):
        ...
        elif self.__class__.__name__ == "Spez_enemy" or self.__class__.__name__ == "Boss_adds":
        ...
        elif self.__class__.__name__ == "Bosses" or self.__class__.__name__ == "Elites":
