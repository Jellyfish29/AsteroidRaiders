...
class Interface:
    ...
    def create():
        for color, location, icon_idx in[
                ...
                ((255, 255, 255), (1880, 910), 4),  # Turret.nuke
                ((255, 255, 255), (1700, 1000), 1),  # Turret.pd
                ((255, 255, 255), (1820, 1000), 1),  # missile ammo
                ((255, 255, 255), (0, 1045), 5)  # FPS
        ]:
...
class Turret:
    ...
    def pd_ammo_reload():
        Turret.pd_ammo += 0.002

    def nuke_reload(boss_kill):
        if boss_kill:
            Turret.nuke_ammo += 1
        else:
            Turret.nuke_ammo += 0.0001

    def missile_reload():
        Turret.missile_ammo += 0.0005
...
class Bosses(Spez_enemy):
    ...
    def __init__(self, typ, health, speed, fire_rate, boss_skill, move_pattern, size):
        ...
        if "main_gun" in self.boss_skill:
            self.laser_ticker = 0
            self.laser_fire_rate = 480
            self.laser_charge_ticker = 0
            self.laser_angles = angles_360(35)
    ...
    def enrage(self):
        if self.health < self.enrage_trigger:
            ...
            if "mines" in self.boss_skill:
                ...
                self.mine_range += 20
            elif "seeker_missiles" in self.boss_skill:
    ...
    def boss_skills(self):
        ...
        if "mines" in self.boss_skill:
            ...
            if self.mine_ticker > self.mine_trigger:
                Bosses.mine_lst.append((pygame.Rect(self.hitbox.center[0], self.hitbox.center[1], 20, 20), pygame.Rect(self.hitbox.center[0] - 125, self.hitbox.center[1] - 125, 250, 250)))
                self.mine_ticker = 0
        ...
        if "volley" in self.boss_skill:
            ...
            if self.volley_ticker > self.fire_rate * 3:
        ...
        if "main_gun" in self.boss_skill:
            self.laser_ticker += 1
            if self.laser_ticker == self.laser_fire_rate / 2:
                Bosses.laser_lst.clear()
            if self.laser_ticker > self.laser_fire_rate:
                self.laser_charge_ticker += 1
                Bosses.laser_lst.append((pygame.Rect(self.hitbox.center[0], self.hitbox.center[1] + self.size[1] / 2, 4, 4), False))
                if "main_gun_2"in self. boss_skill:
                    Bosses.laser_lst.append((pygame.Rect(self.hitbox.center[0], self.hitbox.center[1] - self.size[1] / 2, 4, 4), False))
                if self.laser_charge_ticker > 100:
                    Bosses.laser_lst.append((pygame.Rect(self.hitbox.center[0], self.hitbox.center[1] + self.size[1] / 2, 40, 40), True))
                    if "main_gun_2"in self. boss_skill:
                        Bosses.laser_lst.append((pygame.Rect(self.hitbox.center[0], self.hitbox.center[1] - self.size[1] / 2, 40, 40), True))
                    self.laser_charge_ticker = 0
                    self.laser_ticker = 0
            for idx, (laser, fire) in enumerate(Bosses.laser_lst):
                ...
                if "main_gun_2" in self.boss_skill:
                    ...
                    angle = degrees(Player.hitbox.center[0], self.hitbox.center[0], Player.hitbox.center[1], (self.hitbox.center[1] + self.size[1] / 2))
                laser.move_ip(self.laser_angles[angle])
                if fire:
                    if laser.colliderect(Player.hitbox):
                        Player.hit(1)

    def create(lvl):
        # typ, health, speed, fire_rate, boss_skill, move_pattern, size() Skills: "mines", "seeker_missiles", "salvo"
        if lvl == 2:  # The Corvette
            Bosses.boss_lst.append(Bosses("corv", 180, 5, 90, ["mines", "volley", "seeker_missiles"], (0, 1, 2, 3, 4, 5, 6), (80, 150)))
        elif lvl == 10:  # The Frigate
            Bosses.boss_lst.append(Bosses("FF", 320, 3, 60, ["seeker_missiles", "salvo", "volley"], (0, 1, 2, 3), (100, 180)))
        elif lvl == 15:  # The Destroyerd
            Bosses.boss_lst.append(Bosses("DD", 500, 2, 60, ["seeker_missiles", "mines", "salvo", "volley"], (0, 7), (120, 220)))
        elif lvl == 20:  # The Cruiser
            Bosses.boss_lst.append(Bosses("CA", 800, 1, 50, ["seeker_missiles", "salvo", "main_gun"], (8, 8), (130, 240)))
        elif lvl == 25:  # The Batleship
            Bosses.boss_lst.append(Bosses("BB", 1400, 1, 70, ["salvo", "volley", "main_gun", "main_gun_2"], (8, 8), (140, 260)))
